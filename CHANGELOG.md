...
- **`tools.json`, the tool list as a generated, published artifact.**
  ...
  through a real client, and writes `tools.json` at the package root: name,
  title, description and annotations per tool, sorted by name, with the
  package version and the count. It is generated by `npm run build`
  (`postbuild`), shipped in the npm tarball (`files`) and reachable as
  `@mnemoverse/mcp-memory-server/tools.json` (`exports`), so a downstream
  copy of the surface can be derived from it or checked against it instead
