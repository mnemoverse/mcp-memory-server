...
3. Runs `npm run verify:configs` for drift, and `npm run verify:tools`: `tools.json` (the tool list the `.mcpb` manifest and every count derive from) must equal what the built server registers. `verify:configs` needs no build; `verify:tools` needs `dist/` (the test suite checks the same thing from source, so CI catches a stale `tools.json` on every PR).
